 ...
 const navigate = useNavigate();
  ...
  const quickActions = [
    ...
    },
    {
      ...
      icon: Users,
      gradient: "from-blue-500 to-indigo-500",
      action: () => console.log("Navigate to Add Team"),
      color: "blue",
    },
    {
      title: "View Reports",
      ...
      <div className={`relative z-20 ${isSidebarOpen ? 'w-72' : 'w-20'} transition-all duration-300 backdrop-blur-xl bg-white/5 border-r border-white/10`}>
        <div className="flex flex-col h-full p-4">
          ...
          <nav className="flex-1 space-y-2">
            <button onClick={() => navigate("/admin_home_page")} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-blue-500 text-white shadow-lg shadow-purple-500/50 transition-all">
              <BarChart3 className="w-5 h-5" />
              {isSidebarOpen && <span className="font-medium">Dashboard</span>}
            </button>

            <button onClick={() => navigate("/add-user")}  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-300 hover:bg-white/10 transition-all">
              <UserPlus className="w-5 h-5" />
              {isSidebarOpen && <span className="font-medium">Add User</span>}
            </button>

            <button onClick={() => navigate("/add-team")}  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-300 hover:bg-white/10 transition-all">
